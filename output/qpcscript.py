...
#fig_height = fig_width*golden_mean      # height in inches
    ...
    if len(gedens)<1:
        ...
        pl.imshow(edens[0]/(2*np.pi))
        # im = ax.imshow(edens[0])
        # divider = make_axes_locatable(ax)
        pl.xlabel('$y$ [nm]')
        pl.ylabel('$x$ [nm]')
        pl.text(xshift, 0.5, r'$n$ [1/$m^2$]', fontsize=10,verticalalignment='center',rotation='vertical', transform = pl.gca().transAxes)
...
# spindens = spindens[0]
        ...
        pl.xlabel('$y$ [nm]')
        pl.ylabel('$x$ [nm]')
        pl.text(xshift, 0.5, r'$s_z$ [$\hbar/m^2$]', fontsize=10,verticalalignment='center',rotation='vertical', transform = pl.gca().transAxes)
